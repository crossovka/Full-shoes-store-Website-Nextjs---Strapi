  ...
  data: [
    {
      attributes: {
        ...
        price: 1000,
        original_price: 1200,
        image: { data: ['image1.jpg', 'image2.jpg'] },  // Обновлено, чтобы быть массивом
...
const testProducts = [testProduct, testProduct, testProduct]

interface ProductDetailsProps {
  product: any  // Для упрощения типизацию можно уточнить позже
  products: any  // Здесь тоже
}

const ProductDetails = ({ product, products }: ProductDetailsProps) => {
  const [selectedSize, setSelectedSize] = useState<string | undefined>()
  const [showError, setShowError] = useState(false)
  const p = product?.data?.[0]?.attributes || {} // Защита от возможных ошибок
      ...
      <Wrapper>
        <div className="flex flex-col lg:flex-row md:px-10 gap-[50px] lg:gap-[100px]">
          ...
          <div className="w-full md:w-auto flex-[1.5] max-w-[500px] lg:max-w-full mx-auto lg:mx-0">
            {/* Передаем изображения в компонент */}
